...
func yes_no() rune{
	...
	for {
		...
		if len(answer) == 0 || answer[0] != 'y' && answer[0] != 'n'{
			fmt.Println("\tPlease answer y(es) or n(o)...")
		}else{
...
func play(root *Node) {
	...
	for {
		play(root)
		fmt.Println("Vuoi giocare ancora?")
	}
	...
	if yn == 'y'{
		...
		fmt.Printf("A chi stai pensando? ")
		scanner.Scan()
		rightAnswer := scanner.Text()
		fmt.Printf("Dimmi una domanda in cui la risposta per %s sia si e per %s sia no", leaf.question, rightAnswer)
		scanner.Scan()
